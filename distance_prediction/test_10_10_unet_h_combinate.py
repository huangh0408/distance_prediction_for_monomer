...
def calculate_mae(PRED, YTRUE, pdb_list, length_dict):
    ...
    for i in range(0, len(PRED[:, 0, 0, 0])):
        ...
        P = np.zeros((L, L))
        PRED_name=os.path.join('./predictions/',pdb_list[i]+'.dmap')
        PRED_temp=np.loadtxt(PRED_name)
        # Average the predictions from both triangles (optional)
        # This can improve MAE by upto 6% reduction
        for j in range(0, L):
            for k in range(0, L):
                P[k, j] = (PRED[i, k, j, 0] + PRED[i, j, k, 0]+PRED_temp[k,j]+PRED_temp[j,k]) / 4.0
...
#        avg_mae += sum_mae
    ...
    print('Average ME = %.2f' % (avg_me / len(PRED[:, 0, 0, 0])))

################################################################################
def calculate_mae_hh(PRED, YTRUE, pdb_list, length_dict):
    plot_count = 0
    if flag_show_plots:
        plot_count = 4
    avg_mae = 0.0
...
def calculate_longrange_contact_precision_hh(PRED, YTRUE, pdb_list, length_dict):
    if flag_show_plots:
        plot_count = 4
    avg_precision_L = 0.0
    avg_precision_M = 0.0
...
def write_predictions_to_file(PRED,YTRUE, pdb_list, length_dict, dirpredictions,sequences):
    for i in range(0, len(PRED[:, 0, 0, 0])):
        L = length_dict[pdb_list[i]]
        P = np.zeros((L, L))
        T = np.zeros((L, L))
        ...
        PRED_name=os.path.join('./predictions/',pdb_list[i]+'.dmap')
        PRED_temp=np.loadtxt(PRED_name)
        YTRUE_temp=np.copy(YTRUE[i, 0:L, 0:L, 0])
        # Average the predictions from both triangles (optional)
        # This can improve MAE by upto 6% reduction
        for j in range(0, L):
            for k in range(0, L):
                P[k, j] = (PRED[i, k, j, 0] + PRED[i, j, k, 0]+PRED_temp[k,j]+PRED_temp[j,k]) / 4.0
                T[j,k]=YTRUE_temp[j,k]
                
        f = open(dirpredictions + pdb_list[i] + '_p.txt', 'w')
        f2=open(dirpredictions + pdb_list[i] + '_t.txt', 'w')
        for j in range(0, L):
            for k in range(0, L):
                f.write("%.2f " % P[j, k])
            f.write('\n')
...
#                f.write(P[j, k] + ' ')
 #           f.write('\n')    
        ...
        x=L*L
        DEVIATIONPOSNEG = 0.1
        f = open(dirpredictions + pdb_list[i] + '.rr', 'w')
        f.write(sequences[pdb_list[i]] + '\n')       
        for pair in sorted(p_dict.items(), key=lambda x: x[1]):
            (k, v) = pair
            (i, j) = k
            xx=i+1
            yy=j+1
            f.write("%d %d %.2f %.2f %.5f\n" % (xx, yy, p_dict[k] - DEVIATIONPOSNEG , p_dict[k] + DEVIATIONPOSNEG, 1 / p_dict[k]))
            x -= 1
            if x == 0:
                break
        f.close()
...
#    split = int(0.0 * len(pdb_list))
 #   split2 = int(0.10 * len(pdb_list))
  ...
  #  train_pdbs = pdb_list[split2:split3]
...
#            return -K.sum(alpha * K.pow(1. - pt_1, gamma) * K.log(pt_1))-K.sum((1-alpha) * K.pow( pt_0, gamma) * K.log(1. - pt_0))
            ...
            LM=lm.T
            hhhh=lm+LM
            hhhhh=np.ones((2,256,256,1))
            hhhhh[0,:,:,0]=hhhh
            hhhhh[1,:,:,0]=hhhh
        #    hhh.dtype='float32'
            hhh=np.array(hhhhh,dtype='float32')
            t1=K.log(K.clip(y_pred,K.epsilon(),None)+1.)
            t2=K.log(K.clip(y_true,K.epsilon(),None)+1.)
            return K.mean(K.abs(tf.multiply(hhh,(t1-t2))/K.clip(K.abs(y_true/8+0.5),K.epsilon(),None)),axis=-1)
...
#    model.compile(loss = 'msle', optimizer = 'rmsprop', metrics = ['mae'])
  ...
  #  history = model.fit(XTRAIN, YTRAIN, verbose = 1, batch_size = 2, epochs = 80, validation_data=(XVALID, YVALID), callbacks=[es, mc])
...
#    calculate_mae(P, YTEST, pdb_list, length_dict)
    calculate_mae_hh(P, YTEST, pdb_list, length_dict)
    calculate_mae(P, YTEST, pdb_list, length_dict)
    print('')
    ...
    calculate_longrange_contact_precision_hh(P, YTEST, pdb_list, length_dict)
    # Only if building 3D models
    write_predictions_to_file(P,YTEST, pdb_list, length_dict, dirpredictions,sequence_dict)
